...
def vgnet():
    ...
    try:
    	# 这里开始写你的算法，例如
    	model_path = reconstruct(video_url) # 重建并返回结果地址
...
def reconstruct(img_url):
    ...
    f = open('./configs/shapenet/shapenet_1c_02691156.yaml')
    cfg = yaml.load(f)
    f.close()

    # make model
    model_R = models.Reconstructor('./data/sphere_642.obj').to(device)
    checkpoint = torch.load('./data/models/02691156-R.pth.tar')
    model_R.load_state_dict(checkpoint['model_R'])
    model_R.eval()

    temp_img = misc.imread(filename)
    ...
    srf.save_obj('/home/magic/wuyj/system-packed-algorithm/URA-master/vgnet-demo.obj',vertices[0],faces[0])
    return '/home/magic/wuyj/system-packed-algorithm/URA-master/vgnet-demo.obj'
